...
public class SqlppExpressionToPlanTranslator extends LangExpressionToPlanTranslator
     ...
     * <p>
     * The operator consumes the block's rows and emits one tuple per cluster -- its id, its centre, and
     * its members. That is the whole of what the clause means, so nothing downstream has to rebuild any
     * of it, and whatever produced the rows -- a scan, a join, an UNNEST -- is read exactly once. The
    ...
    public Pair<ILogicalOperator, LogicalVariable> visit(ClusterbyClause clusterbyClause,
        ...
        // CLUSTER AS and naming belongs to this layer. The nested plan below listifies the variable; the
        // expansion adopts that plan, after any aggregate pushdown has rewritten it.
        ...
        memberInput = new MutableObject<>(memberAssign);
        ...
        // The per-row assignment centroid. Seeded here with the vector so every consumer types against ANY;
        // the expansion replaces the definition with the row's entry in the final centroid list.
        LogicalVariable assignedCentroidVar = context.newVar();
        AssignOperator assignedSeed =
                new AssignOperator(assignedCentroidVar, new MutableObject<>(vecRef.cloneExpression()));
        ...
        cop.setAssignedCentroidVariable(assignedCentroidVar);
        // Decorations: the value is read below the operator and bound above it to a fresh variable, which the
        // clauses after the clause then resolve to (as a GROUP BY decoration; the rebinding is scoped to the
        // subplan when the block is a subquery).
        if (clusterbyClause.hasDecorList()) {
        ...
        cop.getInputs().add(memberInput);
        // members as a nested plan, mirroring GROUP AS: the listify is visible to the aggregate-pushdown
        // rule at logical time, so a query that only counts (or otherwise reduces) the members has its
        // aggregate pushed into this plan and the full member list is never materialized.
        NestedTupleSourceOperator membersNts = new NestedTupleSourceOperator(new MutableObject<>(cop));
        membersNts.setSourceLocation(clusterbyClause.getSourceLocation());
        VariableReferenceExpression membersAggArg = new VariableReferenceExpression(memberRecVar);
            ...
            switch (projection.getKind()) {
                ...
                case STAR:
                    ...
                    if (selectBlock.hasGroupbyClause()) {
                        ...
                        getClusterBindings(selectBlock.getClusterbyClause(), fieldBindingsForStar, fieldNamesForStar);
                        if (selectBlock.hasLetHavingClausesAfterGroupby()) {
                            getLetBindings(selectBlock.getLetHavingListAfterGroupby(), fieldBindingsForStar,
                                    fieldNamesForStar, SqlppExpressionToPlanTranslator::includeInSelectStar);
                        }
    ...
    /**
     * What {@code SELECT *} sees after CLUSTER BY: the descriptor and the members, which are the bindings
    ...
    private void getClusterBindings(ClusterbyClause cbc, List<FieldBinding> outFieldBindings, Set<String> outFieldNames)
        ...
        // No descriptor binding, no descriptor field: the star shows only what the query named.
        if (cbc.hasClusterDescriptorVar()) {
            List<FieldBinding> descriptorFields = new ArrayList<>();
            LiteralExpr cidName = new LiteralExpr(new StringLiteral(ClusterByOptions.FIELD_CLUSTER_ID));
            cidName.setSourceLocation(loc);
            descriptorFields.add(new FieldBinding(cidName, new VariableExpr(cbc.getClusterIdVar().getVar())));
            LiteralExpr centroidName = new LiteralExpr(new StringLiteral(ClusterByOptions.FIELD_CENTROID));
            centroidName.setSourceLocation(loc);
            descriptorFields.add(new FieldBinding(centroidName, new VariableExpr(cbc.getCentroidVar().getVar())));
            RecordConstructor descriptorRecord = new RecordConstructor(descriptorFields);
            descriptorRecord.setSourceLocation(loc);
            String scName = SqlppVariableUtil
                    .variableNameToDisplayedFieldName(cbc.getClusterDescriptorVar().getVar().getValue());
            outFieldBindings.add(generateFieldBinding(scName, descriptorRecord, outFieldNames, loc));
        }
        if (cbc.hasClusterMembersVar()) {
            outFieldBindings.add(getFieldBinding(cbc.getClusterMembersVar(), outFieldNames));
        }
    }
