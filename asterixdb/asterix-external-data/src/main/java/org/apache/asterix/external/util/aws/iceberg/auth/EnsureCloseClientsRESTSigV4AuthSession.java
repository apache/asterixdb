...
/**
 * Copy implementation of {@link org.apache.iceberg.aws.RESTSigV4AuthSession} that:
 * - Accepts AwsCredentialsProvider directly to create signed requests.
 */
public class EnsureCloseClientsRESTSigV4AuthSession implements AuthSession {
    ...
    public EnsureCloseClientsRESTSigV4AuthSession(Aws4Signer aws4Signer, AuthSession delegateAuthSession,
    ...
    private HTTPRequest sign(HTTPRequest request) {
        ...
        sdkRequestBuilder.method(SdkHttpMethod.fromValue(request.method().name())).protocol(uri.getScheme()).uri(uri)
    ...
    private Map<String, List<String>> convertHeaders(HTTPHeaders headers) {
        return headers.entries().stream()
                .collect(Collectors.groupingBy(
                        ...
                        header -> header.name().equalsIgnoreCase("Authorization")
                                ? RELOCATED_HEADER_PREFIX + header.name() : header.name(),
            ...
            if (originalHeaders.contains(name)) {
                for (HTTPHeader originalHeader : originalHeaders.entries(name)) {
                    ...
                    if (!signedValues.contains(originalHeader.value())) {
                        newHeaders.addEntry(HTTPHeader.of(RELOCATED_HEADER_PREFIX + name, originalHeader.value()));
                    }
                }
            }
