 ...
 * under the License.
 */
package org.apache.asterix.external.util.google.iceberg.fileio;
...
/**
 * Copy of {@link <a href="https://github.com/apache/iceberg/blob/apache-iceberg-1.10.0/gcp/src/main/java/org/apache/iceberg/gcp/gcs/GCSFileIO.java">GCSFileIO</a>}
 * to support different storage credentials.
 *
 * FileIO Implementation backed by Google Cloud Storage (GCS)
 *
 * <p>Locations follow the conventions used by {@link
 * BlobId#fromGsUtilUri(String) BlobId.fromGsUtilUri} that follow the
 * convention
 *
...
public class GCSFileIO implements DelegateFileIO, SupportsStorageCredentials {
    ...
    private PrefixedStorage clientForStoragePath(String storagePath) {
        ...
        for (String storagePrefix : storageByPrefix().keySet()) {
            if (storagePath.startsWith(storagePrefix) && storagePrefix.length() > matchingPrefix.length()) {
        ...
        Preconditions.checkState(null != client, "[BUG] GCS client for storage path not available: %s", storagePath);
        return client;
    }
    ...
    private void initMetrics(Map<String, String> props) {
        ...
        try {
            DynConstructors.Ctor<MetricsContext> ctor = DynConstructors.builder(MetricsContext.class)
                    .hiddenImpl(DEFAULT_METRICS_IMPL, String.class).buildChecked();
        ...
        if (null == storageByPrefix) {
            synchronized (this) {
                if (null == storageByPrefix) {
                    ...
                    localStorageByPrefix.put(ROOT_STORAGE_PREFIX,
                            new PrefixedStorage(ROOT_STORAGE_PREFIX, properties, storageSupplier));
                    storageCredentials.stream().filter(c -> c.prefix().startsWith(ROOT_STORAGE_PREFIX))
                            .collect(Collectors.toList()).forEach(storageCredential -> {
                                Map<String, String> propertiesWithCredentials = ImmutableMap.<String, String> builder()
                                        .putAll(properties).putAll(storageCredential.config()).buildKeepingLast();

                                localStorageByPrefix.put(storageCredential.prefix(), new PrefixedStorage(
                                        storageCredential.prefix(), propertiesWithCredentials, storageSupplier));
                            });
    ...
    public Iterable<FileInfo> listPrefix(String prefix) {
        ...
        return () -> clientForStoragePath(prefix).storage()
                .list(location.bucket(), Storage.BlobListOption.prefix(location.prefix())).streamAll()
                .map(blob -> new FileInfo(String.format("gs://%s/%s", blob.getBucket(), blob.getName()), blob.getSize(),
    ...
    public void deletePrefix(String prefix) {
        internalDeleteFiles(
                Streams.stream(listPrefix(prefix)).map(fileInfo -> BlobId.fromGsUtilUri(fileInfo.location())));
    }
    ...
    private void internalDeleteFiles(Stream<BlobId> blobIdsToDelete) {
        Streams.stream(Iterators.partition(blobIdsToDelete.iterator(),
                clientForStoragePath(ROOT_STORAGE_PREFIX).gcpProperties().deleteBatchSize())).forEach(batch -> {
                    if (!batch.isEmpty()) {
