...
public class DateTimeValueReader extends AbstractValueReader {
    private final ParquetDeltaBinaryPackingValuesReader dateTimeValueReader;
    private long nextValue;

    public DateTimeValueReader() {
        dateTimeValueReader = new ParquetDeltaBinaryPackingValuesReader();
    }

    @Override
    public void init(AbstractBytesInputStream in, int tupleCount) throws IOException {
        dateTimeValueReader.initFromPage(in);
    }

    @Override
    public void nextValue() {
        nextValue = dateTimeValueReader.readLong();
    }
