...
public class YearMonthDurationValueReader extends AbstractValueReader {
    private final ParquetDeltaBinaryPackingValuesReader yearMonthDurationReader;
    private int nextValue;

    public YearMonthDurationValueReader() {
        yearMonthDurationReader = new ParquetDeltaBinaryPackingValuesReader();
    }

    @Override
    public void init(AbstractBytesInputStream in, int tupleCount) throws IOException {
        yearMonthDurationReader.initFromPage(in);
    }

    @Override
    public void nextValue() throws HyracksDataException {
        nextValue = yearMonthDurationReader.readInteger();
    }
    ...
    public int compareTo(AbstractValueReader o) {
        return Integer.compare(nextValue, o.getInt());
    }
}
