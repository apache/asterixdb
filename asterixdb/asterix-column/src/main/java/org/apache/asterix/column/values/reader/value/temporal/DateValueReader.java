...
public class DateValueReader extends AbstractValueReader {
    private final ParquetDeltaBinaryPackingValuesReader dateReader;
    private int nextValue;

    public DateValueReader() {
        dateReader = new ParquetDeltaBinaryPackingValuesReader();
    }

    @Override
    public void init(AbstractBytesInputStream in, int tupleCount) throws IOException {
        dateReader.initFromPage(in);
    }

    @Override
    public void nextValue() throws HyracksDataException {
        nextValue = dateReader.readInteger();
    }
