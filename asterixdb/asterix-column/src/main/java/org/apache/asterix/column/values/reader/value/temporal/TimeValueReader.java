...
public class TimeValueReader extends AbstractValueReader {
    private final ParquetDeltaBinaryPackingValuesReader timeReader;
    private int nextValue;

    public TimeValueReader() {
        timeReader = new ParquetDeltaBinaryPackingValuesReader();
    }

    @Override
    public void init(AbstractBytesInputStream in, int tupleCount) throws IOException {
        timeReader.initFromPage(in);
    }

    @Override
    public void nextValue() throws HyracksDataException {
        nextValue = timeReader.readInteger();
    }
