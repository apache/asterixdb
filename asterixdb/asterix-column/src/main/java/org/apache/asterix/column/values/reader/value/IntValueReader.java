...
public class IntValueReader extends AbstractValueReader {
    private final ParquetDeltaBinaryPackingValuesReader intReader;
    private final ATypeTag typeTag;
    private int nextValue;

    public IntValueReader(ATypeTag typeTag) {
        intReader = new ParquetDeltaBinaryPackingValuesReader();
        this.typeTag = typeTag;
    }

    @Override
    public void init(AbstractBytesInputStream in, int tupleCount) throws IOException {
        intReader.initFromPage(in);
    }

    @Override
    public void nextValue() throws HyracksDataException {
        nextValue = intReader.readInteger();
    }
