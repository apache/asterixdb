...
/**
 * Cloud-deployment regression test for ASTERIXDB-3788: a large columnar MERGE must flush the
 * compressed .dic (LAF) pages to the append-only cloud writer in ascending page-id order.
 *
 * Runs ONLY the cloud_storage/dic-merge-misalign case, under its own config
 * (cc-cloud-storage-merge.conf.ftl: small buffercache.pagesize + low globalbudget +
 * small column.max.tuple.count) so a merged component's .dic spans multiple LAF pages
 * with little data. Kept separate from {@link CloudStorageTest} so those aggressive
...
public class CloudStorageMergeTest {
    ...
    public static final String SUITE_TESTS = "testsuite_cloud_storage_merge.xml";
    public static final String ONLY_TESTS = "testsuite_cloud_storage_merge_only.xml";
    public static final String CONFIG_FILE_TEMPLATE = "src/test/resources/cc-cloud-storage-merge.conf.ftl";
    public static final String CONFIG_FILE = "target/cc-cloud-storage-merge.conf";
