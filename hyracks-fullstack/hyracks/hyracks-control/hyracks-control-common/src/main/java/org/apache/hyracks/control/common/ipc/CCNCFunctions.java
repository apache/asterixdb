...
public class CCNCFunctions {
    ...
    public static class StartTasksFunction extends Function {
        ...
        public static Object deserialize(ByteBuffer buffer, int length) throws Exception {
            ...
            // read JobKind; absent when the sending CC predates it (mixed-mode rolling upgrade), in
            // which case the stream is already exhausted and the kind is simply unknown. dis wraps a
            // stream bounded by this function's framed length, so available() is exact here.
            JobKind jobKind = null;
            if (dis.available() > 0) {
                int jobKindOrdinal = dis.readByte();
                jobKind = jobKindOrdinal == JOB_KIND_NULL ? null : JobKind.values()[jobKindOrdinal];
            }
