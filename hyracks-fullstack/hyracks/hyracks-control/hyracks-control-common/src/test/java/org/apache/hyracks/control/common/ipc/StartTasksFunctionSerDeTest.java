...
public class StartTasksFunctionSerDeTest {
    ...
    /**
     * A cluster controller that predates the jobKind field writes nothing for it. Such a message must
     * still deserialize on a node controller that does know the field -- during a rolling upgrade the
     * CC may be the older of the two -- yielding an unknown (null) kind rather than an EOFException.
     */
    @Test
    public void missingJobKindIsTolerated() throws Exception {
        byte[] current = serialize(newFunction(JobKind.INGESTION));
        // drop the trailing jobKind byte to reproduce what an older CC would have sent
        byte[] legacy = new byte[current.length - 1];
        System.arraycopy(current, 0, legacy, 0, legacy.length);
        assertNull(deserialize(legacy).getJobKind());
    }
