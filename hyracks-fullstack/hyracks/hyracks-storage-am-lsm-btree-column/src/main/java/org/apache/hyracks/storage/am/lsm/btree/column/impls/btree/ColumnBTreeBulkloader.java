...
public final class ColumnBTreeBulkloader extends BTreeNSMBulkLoader implements IColumnWriteMultiPageOp {
    ...
    /**
     * Ensures that the column writers are initialized and the correct number of column buffers are reserved.
    ...
    /**
     * Reserves the specified number of column buffers from the columnBufferPool.
     * If reservation fails, releases any already held buffers and throws an exception.
     */
     ...
     */
    private boolean isLeafPageFull(ITupleReference tuple) throws HyracksDataException {
        if (tupleCount == 0) {
    ...
    /**
     * Ensures that we have enough column buffers for the required number of columns.
     * Attempts to reserve more buffers if needed.
     * @param requiredColumns The number of columns needed for the current tuple.
     * @return true if sufficient buffers are available or successfully reserved, false otherwise.
     */
    private boolean ensureSufficientBuffers(int requiredColumns) throws HyracksDataException {
        if (reservedBufferCount < requiredColumns) {
            ...
            if (additionalBuffersNeeded > 0) {
                ...
                boolean reserved = columnBufferPool.tryReserve(additionalBuffersNeeded);
                if (reserved) {
                    reservedBufferCount += additionalBuffersNeeded;
                    return true;
                }
                // Not enough buffers available, do not update reservedBufferCount
                return false;
            }
    ...
    /**
     * Checks if the current tuple can fit in the current leaf page based on space requirements.
     * @param tuple The tuple to be added.
     * @return true if the tuple can fit, false otherwise.
     */
    private boolean canCurrentTupleFit(ITupleReference tuple) {
        int requiredSpace = columnWriter.getPageZeroWriterOccupiedSpace(maxColumnsInPageZerothSegment,
                columnarFrame.getBuffer().capacity(), true, pageZeroWriterType);
        requiredSpace += columnWriter.getPrimaryKeysEstimatedSize();
        requiredSpace += lowKey.getTuple().getTupleSize() + getSplitKeySize(tuple);
