...
public class Integer16NormalizedKeyComputerFactory implements INormalizedKeyComputerFactory {
    ...
    public INormalizedKeyComputer createNormalizedKeyComputer() {
        return new INormalizedKeyComputer() {
            ...
            public void normalize(byte[] bytes, int start, int length, int[] normalizedKeys, int keyStart) {
                short value = ShortPointable.getShort(bytes, start);

                // Flip the sign bit so lexicographic order matches numeric order
                int normalized = (value ^ Short.MIN_VALUE) & 0xFFFF;

                normalizedKeys[keyStart] = normalized;
            }
