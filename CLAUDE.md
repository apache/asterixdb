...
- `asterixdb/` — the AsterixDB engine (SQL++ compiler, external data, storage).
- `hyracks-fullstack/` — the Hyracks runtime and shared utilities (`hyracks-util` etc.).
...
(type-level, `REFACTORED`), `hyracks-fullstack/hyracks/hyracks-util/.../SpanTest.java`, and
